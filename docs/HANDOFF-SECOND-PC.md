...
**HEAD على `main`:** (يُحدَّث بعد الدفع) — خطط مدفوعة مغلقة + COD من الطلب + سقف الاسترداد  
...
يجب أن ترى أحدث commit تقريباً: خطط مدفوعة/COD/استرداد (أو أحدث إن وُجدت دفعات لاحقة). `git log -5 --oneline`
